...
<body>
    ...
    <main class="conteudo">
        ...
        <a href="{{ route('salvarResolucao', 'adicao') }}" class="buttonvoltar">&#8592;</a>
            <div class="conteudo-principal-escrito">
                <h1 class="conteudo-principal-escrito-titulo">Jogo de Adição</h1>
                <div class="quiz">
                    <div class="question">
                        <p>{{ $question }}</p>
                    </div>
                    <div class="options">
                        <form method="POST" action="{{ route('conferirResposta', 'adicao') }}">
                            @csrf
                    ...
                    <div class="pontuacao">
                        <p>Pontuação: <span id="score">{{ session('score_adicao', 0) }}</span></p>
                        <p>Acertos: <span id="correct">{{ session('correct_adicao', 0) }}</span></p>
                        <p>Erros: <span id="incorrect">{{ session('incorrect_adicao', 0) }}</span></p>
                    </div>
                    <a href="{{ route('desempenhoDireto', 'adicao') }}" class="botao-desempenho">Ver Desempenho</a>
                </div>
            </div>
